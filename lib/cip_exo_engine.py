...
class ExoType(Enum):
    ...
    SCH3 = "SCH3"        # 硫甲代糖 C5-SCH3 (Thio-methyl, e.g., Lincosamide)
...
def classifyExoSubstituent(
    ...
    for nbr in exoNeighbors:
        ...
        if sym == 7:  # 氮
            ...
            sCount = sum(1 for nn in cNbrNbrs if nn.GetAtomicNum() == 16)
            ...
            if oCount == 0 and sCount == 0 and hCount >= 3:
                return ExoType.CH3.value  # -CH3 (6-deoxy)

            # -CH2SCH3 or similar thio-methyl (Lincomycin)
            if sCount >= 1:
                return ExoType.SCH3.value
    ...
    if hasS:
        return ExoType.SCH3.value  # 直连 S (direct S on ring C)
    if hasN:
        return ExoType.NH2.value
    if hasO and not hasC:
        return ExoType.OH.value

    # DEBUG: if OTHER, print why
    import sys
    print(f"DEBUG classifyExo OTHER: hasC={hasC}, hasO={hasO}, hasN={hasN}, hasS={hasS}", file=sys.stderr)
    for nbr in exoNeighbors:
        print(f"  neighbor: {nbr.GetSymbol()}, idx: {nbr.GetIdx()}", file=sys.stderr)
        if nbr.GetAtomicNum() == 6:
            hCount = nbr.GetTotalNumHs()
            print(f"  Carbon neighbor Hs: {hCount}", file=sys.stderr)
            for nn in nbr.GetNeighbors():
                print(f"    sub-neighbor: {nn.GetSymbol()}", file=sys.stderr)
...
def extractSugarFingerprint(
    ...
    maskMol = Chem.RWMol(mol)
    
    def mute_descendants(start_idx, exclude_idx):
        stack = [start_idx]
        ...
        while stack:
            ...
            visited.add(curr)
            a = maskMol.GetAtomWithIdx(curr)
            if curr != start_idx:
                a.SetAtomicNum(1) # Force to Hydrogen (lowest priority)
            for nbr in a.GetNeighbors():
                if nbr.GetIdx() not in visited and nbr.GetIdx() not in ringSet:
                    stack.append(nbr.GetIdx())

    # Terminal Normalization: Protect ring stereocenters from heavy side chains
    for cIdx in path[:-1]: # C1 to C4 (or C1 to C3 for furanose)
        a = maskMol.GetAtomWithIdx(cIdx)
        for nbr in a.GetNeighbors():
            if nbr.GetIdx() not in ringSet:
                nbr.SetAtomicNum(8) # Force immediate exocyclic atom to Oxygen
                mute_descendants(nbr.GetIdx(), cIdx)
                
    # Normalize terminal C (C5 in pyranose, C4 in furanose)
    terminal_C = path[-1]
    terminal_atom = maskMol.GetAtomWithIdx(terminal_C)
    exo_terminal = classifyExoSubstituent(mol, terminal_C, ringSet)
    
    for nbr in terminal_atom.GetNeighbors():
        if nbr.GetIdx() not in ringSet:
            nbr.SetAtomicNum(6) # Force C6 to Carbon
            c6_nbrs = [nn for nn in nbr.GetNeighbors() if nn.GetIdx() != terminal_C]
            if exo_terminal in [ExoType.CH2OH.value, ExoType.COOH.value, ExoType.SCH3.value, ExoType.NH2.value, ExoType.OTHER.value]:
                if c6_nbrs:
                    c6_nbrs[0].SetAtomicNum(8) # Give it one Oxygen to maintain priority
                    mute_descendants(c6_nbrs[0].GetIdx(), nbr.GetIdx())
                    for other in c6_nbrs[1:]:
                        other.SetAtomicNum(1) # Mute the rest
                        mute_descendants(other.GetIdx(), nbr.GetIdx())
            else:
                # Deoxy (CH3) or terminal H -> all neighbors of C6 muted
                for other in c6_nbrs:
                    other.SetAtomicNum(1)
                    mute_descendants(other.GetIdx(), nbr.GetIdx())

    # 强制 CIP 计算 (Force CIP assignment on the NORMALIZED molecule)
    Chem.AssignStereochemistry(maskMol, force=True, cleanIt=True)

    # 提取各位置 CIP (Extract CIP at each position)
    def getCip(atomIdx: int) -> str:
        a = maskMol.GetAtomWithIdx(atomIdx)
